...
import { Network, ChevronDown, Copy, Check, Download, ExternalLink, Loader2, RotateCcw, FileText } from "lucide-react";
import { CompetitorTab } from "@/components/blueprint/competitor-tab";
...
import { HistoryDropdown } from "@/components/blueprint/history-dropdown";
import { ShareButton } from "@/components/blueprint/share-button";
import { createClient } from "@/lib/supabase/client";
import { cn } from "@/lib/utils";
import type { Tag, BRDMetrics, MarketMetrics, BlueprintResult } from "@/lib/schemas";
import type { BlueprintRow, CompetitorRow } from "@/lib/types";

// Re-export for backwards compat with any child components that import from here
...
type AppState = {
  ...
  activeTab: "brd" | "prd" | "market" | "competitor";
  ...
  savedMarket: string;
  isDirty: boolean;
  // ── Competitor deep-dive (Step 8) ─────────────────────────────────────────
  competitorData: CompetitorRow[] | null;
  isDeepDiveLoading: boolean;
  deepDiveError: string | null;
};

// Extended API response type that includes the Supabase row ID
type GenerateApiResponse = BlueprintResult & { blueprintId?: string | null };
...
const TAB_NAMES: Record<"brd" | "prd" | "market" | "competitor", string> = {
...
export default function AppPage() {
  ...
  const apiDataRef = useRef<GenerateApiResponse | null>(null);
  const transitionedRef = useRef(false);

  // ── Fetch logged-in user info for avatar ─────────────────────────────────

  useEffect(() => {
    const supabase = createClient();
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return;
      const av: string | undefined =
        (user.user_metadata?.avatar_url as string | undefined) ??
        (user.user_metadata?.picture as string | undefined);
      if (av) setUserAvatarUrl(av);
      const name: string =
  ...
  // ── Load blueprint from ?load= URL param ─────────────────────────────────
  ...
  useEffect(() => {
    ...
    fetch(`/api/blueprints/${loadId}`)
      .then((r) => r.json())
      .then((json: { blueprint?: BlueprintRow }) => {
        if (json.blueprint) handleLoadBlueprint(json.blueprint);
      })
      .catch(() => {/* silent — user lands on Screen 1 */});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = (await resp.json()) as GenerateApiResponse;
        if (!cancelled) {
    ...
    const tid = setTimeout(() => {
      setAppState((prev) => {
        ...
        return {
          ...
          isPublic: false,
          savedBRD: data.brd.markdown,
          savedPRD: data.prd.markdown,
          savedMarket: data.market.markdown,
          isDirty: false,
          competitorData: null,
        };
      });
  ...
  function handleReset() {
    ...
    setShowEmptyHint(false);
    setCopied(false);
    // INITIAL_STATE already zeroes blueprintId, isPublic, isDirty, competitorData
  }

  function handleLoadBlueprint(bp: BlueprintRow) {
    transitionedRef.current = false;
    setAppState({
      ...INITIAL_STATE,
      ideaInput: bp.idea_input ?? "",
      generatedBRD: bp.brd ?? "",
      generatedPRD: bp.prd ?? "",
      generatedMarket: bp.market ?? "",
      brdTitle: bp.title ?? "",
      savedBRD: bp.brd ?? "",
      savedPRD: bp.prd ?? "",
      savedMarket: bp.market ?? "",
      blueprintId: bp.id,
      isPublic: bp.is_public,
      competitorData: bp.competitor_data ?? null,
      currentScreen: 3,
    });
  }

  async function handleMakePublic() {
    if (!appState.blueprintId) return;
    await fetch(`/api/blueprints/${appState.blueprintId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ is_public: true }),
    });
    setAppState((prev) => ({ ...prev, isPublic: true }));
  }
  ...
  async function handleMakePrivate() {
    ...
    setAppState((prev) => ({ ...prev, isPublic: false }));
  }
  ...
  function getActiveTabMarkdown(): string {
    ...
    if (appState.activeTab === "market") return appState.generatedMarket;
    return "";
  }
  ...
  async function handleDownloadPDF() {
    ...
    await downloadBlueprintPDF({
      title: appState.brdTitle || appState.ideaInput.slice(0, 60),
      brd: appState.generatedBRD,
      prd: appState.generatedPRD,
      market: appState.generatedMarket,
    });
  }
  ...
  async function handleRegenerate() {
    if (appState.activeTab === "competitor") return;
  ...
  function renderNavBar() {
    return (
      <nav className="h-12 border-b flex items-center justify-between px-4 shrink-0 bg-background">
        ...
        <div className="flex items-center gap-2">
          {/* History dropdown — only relevant when logged in */}
          <HistoryDropdown
          ...
          <Link
            href="/profile"
            title="My profile"
            className="flex items-center justify-center h-7 w-7 rounded-full border border-border bg-muted overflow-hidden hover:ring-2 hover:ring-ring transition-all"
          >
            {userAvatarUrl ? (
  ...
  function renderScreen3() {
    const isCompetitorTab = appState.activeTab === "competitor";
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="h-10 border-b flex items-center justify-between px-4 shrink-0">
            ...
            <div className="flex items-center gap-1">
              ...
              <ShareButton
                blueprintId={appState.blueprintId}
          ...
          {isCompetitorTab ? (
            <CompetitorTab
              blueprintId={appState.blueprintId}
              marketMarkdown={appState.generatedMarket}
              brdMarkdown={appState.generatedBRD}
              knownCompetitorsCount={appState.marketMetrics.knownCompetitors}
              savedCompetitorData={appState.competitorData}
              onComplete={(results) =>
                setAppState((prev) => ({ ...prev, competitorData: results }))
              }
            />
          ) : (
          ...
          {/* Refinement row — hidden on competitor tab */}
          {!isCompetitorTab && (
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Clean A4 document — cover page, business case, product specs, market intel
                </p>
              </div>
            </button>
